        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
          ...
          <div class="md:col-span-1">
            ...
            <div class="mt-2 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-2 lg:grid-cols-5 gap-2">
                <button v-for="freq in frequencyOptions" :key="freq" type="button" @click="formData.frequency = freq" :class="['rounded-md py-1 md:py-2 px-1 md:px-2 text-xs md:text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-600', formData.frequency === freq ? 'bg-gray-800 text-xs md:text-sm text-white hover:bg-gray-900' : 'bg-white text-gray-700 border border-gray-500 hover:bg-gray-50']">{{ freq }}</button>
            </div>
        </div>
